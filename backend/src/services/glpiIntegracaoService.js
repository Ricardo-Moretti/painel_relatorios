...
const glpiIntegracaoService = {
  ...
  async relatorioDiario() {
    ...
    const [solucionadosOntem] = await p.execute(`SELECT COUNT(*) as total FROM glpi_tickets t ${GF} WHERE t.solvedate IS NOT NULL AND DATE(t.solvedate) = DATE_SUB(CURDATE(), INTERVAL 1 DAY) AND t.is_deleted = 0 ${EF}`);
